package com.ft.base.http.interceptor.logging;
...
//        List<String> segmentList = ((Request) request.tag()).url().encodedPathSegments();
        ...
        if (subtype != null && (subtype.contains("json")
            ...
            Log.e("ResponseBody","bodyString=="+bodyString);
            String bodyJson = Printer.getJsonString(bodyString);
            Log.e("ResponseBody","bodyJson=="+bodyJson);
            Printer.printJsonResponse(builder, chainMs, isSuccessful, code, header, bodyJson, null);
            body = ResponseBody.create(contentType, bodyString);
        } else {
            Printer.printFileResponse(builder, chainMs, isSuccessful, code, header, null);
            return response;
        }
