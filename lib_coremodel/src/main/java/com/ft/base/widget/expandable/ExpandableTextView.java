package com.ft.base.widget.expandable;
...
public class ExpandableTextView extends AppCompatTextView {
    ...
    public static final String ELLIPSIS_STRING = new String(new char[]{'\u2026'}); //三个点
    ...
    private boolean mOpenInNewLine;
    @Nullable
    ...
    private boolean isClick = false; // 是否点击展开、收起
    ...
    private void initialize() {
        mOpenSuffixColor = mCloseSuffixColor = Color.parseColor("#f67133");
    ...
    public void setOriginalText(CharSequence originalText) {
        boolean empty = StringUtils.isEmpty(originalText);
        if (empty){
            originalText = "";
        }
...
//                SpannableStringBuilder tempText2 = charSequenceToSpannable(mCloseSpannableStr).append(ELLIPSIS_STRING);
                SpannableStringBuilder tempText2 = charSequenceToSpannable(mCloseSpannableStr).append("你、\n");
...
//                    tempText2 = charSequenceToSpannable(mCloseSpannableStr).append(ELLIPSIS_STRING);
                    tempText2 = charSequenceToSpannable(mCloseSpannableStr); //去掉三个点
                    if (mOpenSuffixSpan != null) {
                        tempText2.append(mOpenSuffixSpan);
                    }
                ...
                int lastSpace = mCloseSpannableStr.length() - mOpenSuffixSpan.length()-1;
...
private Layout createStaticLayout(SpannableStringBuilder spannable) {
     ...
     * 展开是否另起一行
     */
    public void setOpenInNewLine(boolean openInNewLine) {
        mOpenInNewLine = openInNewLine;
        updateOpenSuffixSpan();
    }
...
//        Log.e("txt","mOpenSuffixStr.length()=="+mOpenSuffixStr.length());
        AlignmentSpan alignmentSpan = new AlignmentSpan.Standard(Layout.Alignment.ALIGN_NORMAL);
        mOpenSuffixSpan.setSpan(alignmentSpan, 0, 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//            AlignmentSpan alignmentSpan = new AlignmentSpan.Standard(Layout.Alignment.ALIGN_OPPOSITE);
    ...
    public void onTextViewClick(final BindingCommand clickCommand) {
        this.clickCommand = clickCommand;
       /* RxView.clicks(ExpandableTextView.this)
